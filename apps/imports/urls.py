...
urlpatterns = [
    ...
    path('<uuid:job_id>/preview/', views.preview_file, name='preview_file'),
    
    # CSV_2: Mapping & Schema
    path('schema/<str:entity>/', views.get_entity_schema, name='entity_schema'),
    path('<uuid:job_id>/auto-map/', views.auto_map_columns, name='auto_map_columns'),
    
    # CSV_4: Status & Polling
    path('<uuid:job_id>/status/', views.job_status, name='job_status'),
